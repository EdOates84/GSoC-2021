...
def segment_video(vid_path, output_path, verbose=False):
    ...
    if os.path.exists(output_path+'/video_encodings.pickle'):
      print('yes_path_exist')
      infile = open(output_path+'/video_encodings.pickle','rb')
      allEncodings = pickle.load(infile)
    else:
      path = os.path.join(output_path, 'video_encodings.pickle')
      # os.mkdir(path)
      pickle_file = open(path,'wb')
      allEncodings = findFaces(vid_path)
      pickle.dump(allEncodings,pickle_file)
    # allEncodings = findFaces(vid_path)
    ...
    vid_txt3_path = '/content/drive/My Drive/Datasets/S2-H8 encodings/2006-01-05_0000_US_00000070_V2_MB2_VHS2_H8_MS.txt3'
